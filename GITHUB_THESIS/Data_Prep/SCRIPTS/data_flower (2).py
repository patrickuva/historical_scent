...
class DataLoader:
    ...
    **Custom Data**
    ...
    def load_docs(
        ...
        ```python
        dataloader = DataLoader(dataset="EN")
        ...
        if self.dataset == "EN":
            self.docs, self.timestamps = self._EN()
        elif self.dataset == "EN_dtm":
            self.docs, self.timestamps = self._EN_dtm()
    ...
    def _EN(self) -> Tuple[List[str], List[str]]:
        """Prepare the NL dataset"""
        EN = pd.read_csv("full_flowers.csv", quoting=csv.QUOTE_NONE)
        
        # Drop rows where 'modern_sentences' is NaN
        EN = EN.dropna(subset=['pre_text'])
        EN = EN.dropna(subset=['year'])
        
        EN['pre_text'] = EN['pre_text'].astype(str)
        EN['year'] = EN['year'].astype(float)
        
        timestamps = EN.year.to_list()
        docs = EN.pre_text.to_list()
    ...
    def _EN_dtm(self) -> Tuple[List[str], List[str]]:
        """Prepare the EN dataset including timestamps"""
        EN = pd.read_csv("full_flowers.csv", quoting=csv.QUOTE_NONE)
        ...
        EN = EN.explode("pre_text").sort_values("year")
        #dataset = dataset.loc[dataset.year > 2005, :]
        docs = EN.pre_text.tolist()
        timestamps = EN.year.tolist()
        return docs, timestamps
