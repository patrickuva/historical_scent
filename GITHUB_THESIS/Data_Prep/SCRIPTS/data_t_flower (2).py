...
class DataLoader:
    ...
    **Custom Data**
    ...
    def _EN(self) -> Tuple[List[str], List[str]]:
        """Prepare the EN dataset"""
        EN = pd.read_csv("10k_EN_mod.csv")
        
        # Drop rows where 'modern_sentences' is NaN
        EN = EN.dropna(subset=['pre_mod'])
        # Drop rows where 'modern_sentences' is NaN
        EN = EN.dropna(subset=['year'])
        
        
        timestamps = EN.year.to_list()
        docs = EN.pre_mod.to_list()
    ...
    def _EN_dtm(self) -> Tuple[List[str], List[str]]:
        ...
        #EN["text"] = EN.apply(lambda row: create_paragraphs(row.pre_text), 1)
        EN = EN.explode("pre_mod").sort_values("year")
        #dataset = dataset.loc[dataset.year > 2005, :]
        docs = EN.pre_mod.tolist()
