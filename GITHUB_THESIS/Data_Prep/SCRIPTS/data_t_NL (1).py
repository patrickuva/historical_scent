...
class DataLoader:
    ...
    **Custom Data**
    ...
    def _NL_dtm(self) -> Tuple[List[str], List[str]]:
        ...
        NL = NL.explode("pre_mod").sort_values("year")
        #dataset = dataset.loc[dataset.year > 2005, :]
        docs = NL.pre_mod.tolist()
...
#         """Prepare the NL dataset"""
#         # Read the CSV file
#         NL = pd.read_csv("translated_sentences_10k_NL.csv")
#         NL['pre_text'] = NL['pre_text'].str.lower()
        
#         # Drop rows where 'pre_text' is NaN
...
#         df["Bins"] = pd.cut(df["Timestamp"], bins=nr_bins, right=True)
#         df["Timestamp"] = df["Bins"] 
        
#         # Drop duplicates
#         df.drop_duplicates(subset=["Doc", "Timestamp"], inplace=True)
